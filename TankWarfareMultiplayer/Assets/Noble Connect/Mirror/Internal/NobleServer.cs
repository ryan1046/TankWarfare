...
namespace NobleConnect.Mirror
{
    ...
    public class NobleServer
    {
        ...
        public static void SendToReady<T>(NetworkIdentity identity, T msg, bool includeSelf = true, int channelId = Channels.Reliable) where T : struct, NetworkMessage
        {
            NetworkServer.SendToReady<T>(identity, msg, includeSelf, channelId);
        }
        public static void SendToReady<T>(NetworkIdentity identity, T msg, int channelId = Channels.Reliable) where T : struct, NetworkMessage
        {
